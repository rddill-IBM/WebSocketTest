 ...
 */
function wsDisplay(_target)
{
    let content = $('#'+_target);
    let wsSocket = new WebSocket('ws://'+host_address);
    wsSocket.onopen = function ()
    {console.log ('wsSocket.onOpen initiated to: '+host_address); wsSocket.send('connected to client');};
    wsSocket.onmessage = function (message)
    {
        console.log('wsDisplay', message);
        switch (message.type)
        {
        case 'Message':
        ...
        case 'Alert':
            ...
            addNotification(message.data.type, message.data.ID, message.data.orderID);
            break;
        case 'BlockChain':
            _blctr ++;
            ...
            break;
        default:
            console.log('Can Not Process message type: ',message.type);
        }
