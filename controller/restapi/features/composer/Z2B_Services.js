 ...
 */
    send: function (_locals, type, event)
    {
        ...
        util.displayObjectValuesRecursive ('_locals.sockets.clients: ',_locals.sockets.clients, 5);
        if (_locals.sockets.clients.length === 'undefined')
        {_locals.sockets.clients.send({'type': type, 'data': event} );}
        else
        {
            for( let c = 0; c < _locals.sockets.clients.length; c++ )
            {(function (_c, _type, _event)
                {_locals.sockets.clients[_c].send({'type': _type, 'data': _event} );})(c, type, event);
            }
        }
    }
};
